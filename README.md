...
导出的离线单文件 HTML（量子迷宫手势交互页）如果直接双击用 `file://` 打开，浏览器安全策略会禁止调用摄像头，页面会显示"未授权或无可用摄像头"。必须通过 `http://localhost` 打开才能授权摄像头（无需联网）。

仓库提供了一键工具：把 `离线运行工具/` 里的 `启动本地服务器.bat` 和 `server.ps1` 复制到离线 HTML 所在文件夹，双击 `.bat` 即可自动启动本地服务器并打开浏览器（优先用 Python，没装 Python 时自动改用 Windows 自带的 PowerShell）。详见 [`离线运行工具/使用说明.md`](离线运行工具/使用说明.md)。
